...
function Main() {
  ...
  const carregarTransacoes = async () => {
    try {
      const dados = await (await fetch("http://localhost:3333/transactions", {method: "GET"})).json();
  ...
  const cadastrarTransacoes = async () => {
    ...
    try {
    await fetch("http://localhost:3333/transactions", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(body)});
  ...
  const editarTransacao = async (transId) => {
    ...
    try {
      await fetch("http://localhost:3333/transactions/" + transId, { method: "PUT", headers: { "Content-Type": "application/json"}, body: JSON.stringify(body)});
  ...
  const deletarTransacao = async (transId) => {
    try {
      await fetch("http://localhost:3333/transactions/" + transId, { method: "DELETE"});
